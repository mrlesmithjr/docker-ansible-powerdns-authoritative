...
ansible-playbook -i "localhost," -c local \
  ...
  pdns_allow_ddns_update=$PDNS_ALLOW_DDNS_UPDATE \
  pdns_json_interface=$PDNS_JSON_INTERFACE \
  pdns_gmysql_dbname=$PDNS_GMYSQL_DBNAME \
  ...
  pdns_recursor_server_ip=$PDNS_RECURSOR_SERVER_IP \
  pdns_recursor_server_port=$PDNS_RECURSOR_SERVER_PORT \
